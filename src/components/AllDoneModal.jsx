...
import SpinWheel, { SECTIONS } from './SpinWheel'
...
export default function AllDoneModal({ onClose }) {
  const [phase, setPhase] = useState('wheel') // 'wheel' | 'prize'
  ...
  // Gentle rain the whole time the modal is open
  useEffect(() => {
    const stop = startConfettiRain()
    return stop
  }, [])

  const handleResult = (idx) => {
    setWinner(idx)
  }
  ...
  // Big bursts on the prize screen
  useEffect(() => {
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === 'wheel' && (
          ...
          >
            ...
            {/* Header */}
            <motion.div
              className="text-6xl mb-2 flex gap-2"
            ...
            <motion.div
              className="text-yellow-300 text-xl font-black mb-6"
            ...
            {/* Claim button — appears after wheel stops */}
            <AnimatePresence>
              {winner !== null && (
                <motion.button
                  onClick={handleClaim}
                  ...
                  style={{
                    ...
                    background: `linear-gradient(135deg, ${SECTIONS[winner].color}, ${SECTIONS[winner].dark})`,
        ...
        {phase === 'prize' && winSection && (
          <motion.div
            ...
            style={{
              background: `radial-gradient(ellipse at center, ${winSection.color} 0%, ${winSection.dark} 50%, #000 100%)`,
            }}
          >
            {/* Floating emojis */}
            {['🎊', '🎉', '⭐', '🌟', '✨', '💫', '🎈', '🏆'].map((e, i) => (
            ...
            {/* Team name */}
            <motion.div
              className="text-white text-5xl relative z-10 mb-1 flex gap-3 items-center"
              style={{ fontFamily: "'Fredoka One', cursive", textShadow: '0 0 30px rgba(255,255,255,0.6)' }}
              initial={{ y: 40, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: 0.1, type: 'spring' }}
            >
              <span>👧</span> Peyton &amp; Jude <span>👦</span>
            </motion.div>

            {/* Win message */}
            <motion.div
              className="text-yellow-300 text-3xl relative z-10 mb-2 text-center"
            ...
            {/* Prize reveal */}
            <motion.div
              className="relative z-10 rounded-3xl px-8 py-5 mb-8 text-center"
              style={{
                background: 'rgba(255,255,255,0.15)',
                backdropFilter: 'blur(8px)',
                border: `3px solid ${winSection.color}`,
                boxShadow: `0 0 40px ${winSection.color}88`,
              }}
              initial={{ scale: 0, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
            ...
            >
              <div
                className="text-white/70 text-lg mb-1"
              ...
              <motion.div
                ...
                style={{
                  fontFamily: "'Fredoka One', cursive",
                  textShadow: `0 0 30px ${winSection.color}`,
                }}
